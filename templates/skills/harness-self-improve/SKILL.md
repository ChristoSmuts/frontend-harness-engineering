...
3. Edit **canonical** harness only (`{{CANONICAL_SKILLS_DIR}}` for skills; `.cursor/rules/` for Cursor rules)
4. Do **not** duplicate guidance into `AGENTS.md` (keep <= 60 lines)
5. For a new project skill, run `{{HARNESS_SCRIPTS_DIR}}/register-harness-growth.sh --kind skill --name <kebab> --when "<when>" --summary "<what>"` (or `.ps1` on Windows)
6. For orchestration index rows: `{{HARNESS_SCRIPTS_DIR}}/register-harness-growth.sh --kind orchestration-row --name <skill> --when "<when>"`
7. Run `{{HARNESS_SCRIPTS_DIR}}/sync-skills.sh --all-mirrors --orchestration` (or `sync-skills.ps1 -AllMirrors -Orchestration`)
8. Run `{{HARNESS_SCRIPTS_DIR}}/validate-target-harness.sh` (or `.ps1`); fix errors before finishing
...
Follow existing skills in `{{CANONICAL_SKILLS_DIR}}` for section layout.
...
- Canonical skills: `{{CANONICAL_SKILLS_DIR}}`
